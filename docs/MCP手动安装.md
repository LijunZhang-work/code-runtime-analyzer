...
```bash
npm install --global /mnt/c/Users/你的用户名/Downloads/Code-Runtime-Analyzer-MCP-v0.10.2.tgz
```

如果 OpenCode 直接运行在 Windows 中，就在 PowerShell 执行：

```powershell
npm install --global .\Code-Runtime-Analyzer-MCP-v0.10.2.tgz
```
...
然后在 OpenCode 配置中添加网页“AI 与扩展”页面提供的配置。网页给出的内容包含后台这一次真实使用的端口和随机访问密钥，因此不要照抄文档里的固定端口。后台重新启动后，如果 OpenCode 提示密钥失效，再从网页复制一次即可。EXE 不会替你写入配置，也不会覆盖另一个项目已有的 MCP。
