...
这篇写给准备提交代码的人。它只回答“哪些文件能上传、哪些不能上传”，以及如何让 GitHub 自动生成普通用户可安装的 `.vsix`；普通使用者不需要阅读。
...
普通用户不下载源码。正式发布时，只需要在已经检查通过的提交上创建一个版本标签，例如 `v0.9.2`，并把该标签推送到 GitHub。

GitHub Actions 会自动完成下面三件事：

1. 安装构建依赖、构建网页和扩展；
2. 生成一个 `Code-Runtime-Analyzer-v0.9.2.vsix` 安装包；
3. 在 GitHub 的 **Releases** 页面创建对应版本，并把安装包放进去。

这样普通用户只需下载 `.vsix`，在 VS Code 中选择“从 VSIX 安装”。版本号只在真正要发给用户的新安装包时才增加，不因为日常的小修改频繁变化。
