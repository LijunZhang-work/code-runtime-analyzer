...
安装 EXE 后，可以从 Windows 开始菜单打开 `Code Runtime Analyzer → 后台控制中心`。这是一个不依赖 VBS、Electron 或额外 .NET 的原生 Windows 窗口，可以查看状态、启动、停止、重启、打开 Web、查看日志或导出诊断。只看见 Web 页面不代表扩展或 MCP 已经安装；它们是分开的组件。

后台会优先使用本机端口 47831；如果它已被占用，会自动换到其他可用端口。实际地址和每次启动生成的随机访问密钥保存在当前 Windows 用户的本机状态中，扩展会自动读取，用户不用改端口。Web 必须从后台控制中心或已连接的编辑器打开；WSL 中的 OpenCode 使用网页生成的当次配置。

如果暂时只安装 VSIX，扩展也能使用：默认的“自动”模式会先寻找独立后台，找不到时启动扩展自带后台，不会因为没有安装 EXE 而反复弹出健康检查错误。要让 OpenCode、长期运行的 Web 工作台或多个编辑器窗口共享同一个服务，仍然需要安装并启动 EXE 提供的独立后台。
...
| 我要确认换一台电脑是否还能用、后面还要补什么 | [跨电脑通用化方案](docs/跨电脑通用化方案.md) |
...
提交一个形如 `v0.10.2` 的版本标签后，GitHub 会自动制作 Release，同时生成 Windows 后台 EXE、编辑器 VSIX 和独立 MCP 包。维护流程见 [GitHub 发布清单](docs/GitHub发布清单.md)。如果 Release 下载很慢，也可以下载源码后运行一条本地构建命令，见 [从源码生成安装包](docs/从源码生成安装包.md)。
